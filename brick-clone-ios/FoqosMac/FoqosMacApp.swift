...
struct FoqosMacApp: App {
  ...
  init() {
    ...
    MacDiagnostics.shared.record(
      "app.launched", "BePresent Mac launched; local diagnostics are active.", fields: appFields)
        ...
        Task { @MainActor in
          filterManager.resetForDevelopment { result in
            ...
            case .success(let requiresRestart):
              if requiresRestart {
                print("BePresent reset is pending. Restart this Mac to finish removing the extension.")
              } else {
                print(
                  "BePresent filter configuration was removed and the system extension was deactivated."
                )
              }
              exit(EXIT_SUCCESS)
            case .failure(let error):
              fputs("Unable to reset BePresent: \(error.localizedDescription)\n", stderr)
              exit(EXIT_FAILURE)
            }
          }
        }
  ...
  var body: some Scene {
    MenuBarExtra {
      ...
      Label("BePresent", systemImage: menuBarSystemImage)
    }
