...
struct MenuBarContentView: View {
  ...
  private var actions: some View {
    VStack(alignment: .leading, spacing: 0) {
      ...
      .help("Refresh BePresent state")

      DiagnosticsExportButton(isMenuItem: true)

      MenuActionItem(title: "Quit BePresent") {
        controller.quit()
      }
      .help("Quit BePresent")
    }
  ...
  private var updaterHelpText: String {
    if updaterController.isConfigured {
      return "Check GitHub Releases for a newer version of BePresent"
    }
  ...
  private var statusTitle: String {
    ...
    case .breakActive:
      return "BePresent break is active"
    case .paused:
      return "BePresent is paused"
    default:
      return "No active BePresent profile"
    }
  }
