...
struct ProfileWidgetEntryView: View {
    ...
    Group {
      ...
      case .accessoryInline:
        Label {
          if let start = entry.sessionStartTime, !entry.isPauseActive, !entry.isBreakActive {
            ...
            Text(entry.isSessionActive ? entry.statusLabel : entry.profileName ?? "BePresent")
          }
        } icon: {
      ...
      case .accessoryRectangular:
        VStack(alignment: .leading, spacing: 3) {
          Text(entry.profileName ?? "BePresent").font(.headline).lineLimit(1)
  ...
  private func status(compact: Bool) -> some View {
    HStack(spacing: 5) {
      ...
      if !compact {
        Spacer(minLength: 0)
        Text("BePresent").font(.system(size: 11, weight: .bold, design: .rounded))
