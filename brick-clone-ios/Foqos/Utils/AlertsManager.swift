    ...
    if authorizationStatus != .approved {
      updatedAlerts.append(
        HomeAlert(
          ...
          detailMessage:
            "BePresent needs Screen Time access to block apps and websites. Grant access again to restore blocking.",
      ...
      contentsOf:
        ...
        .map { profile in
          HomeAlert(
            ...
            detailMessage:
              "\(profile.name)'s schedule is saved, but iOS is no longer monitoring it. This usually can happen when you combine foqos with other blocking apps, it is recommended to turn those apps off.",
