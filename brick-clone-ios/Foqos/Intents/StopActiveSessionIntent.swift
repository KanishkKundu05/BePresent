...
struct StopActiveSessionIntent: AppIntent {
  ...
  static var title: LocalizedStringResource = "Stop Active BePresent Session"
  static var description = IntentDescription(
    "Attempts to stop any currently active BePresent session."
  )
  ...
  func perform() async throws -> some IntentResult & ReturnsValue<Bool> & ProvidesDialog {
    ...
    guard let blockedProfile = strategyManager.activeSession?.blockedProfile,
      ...
      return .result(value: true, dialog: "No active BePresent session to stop")
    }
