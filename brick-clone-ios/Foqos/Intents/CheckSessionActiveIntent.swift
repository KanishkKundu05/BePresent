...
struct CheckSessionActiveIntent: AppIntent {
  ...
  static var title: LocalizedStringResource = "Check if BePresent Session is Active"
  static var description = IntentDescription(
    "Check if any BePresent blocking session is currently active and return true or false. Useful for automation and shortcuts."
  )
  ...
  func perform() async throws -> some IntentResult & ReturnsValue<Bool> & ProvidesDialog {
    ...
    let dialogMessage =
      isActive
      ? "A BePresent session is currently active."
      : "No BePresent session is active."
