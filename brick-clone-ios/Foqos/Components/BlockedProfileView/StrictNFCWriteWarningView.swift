...
struct StrictNFCWriteWarningView: View {
  ...
  private var strictModeWarning: Text {
    ...
    return baseText
      ...
      .fontWeight(.bold)
      + Text(
        ". Rewriting it can change what BePresent reads later and may stop this profile from unlocking. You do not need to create another NFC tag for this profile."
      )
  }

  private var convenienceText: String {
    return
      "This only creates a convenience shortcut: scan the tag, tap the iOS notification, and BePresent opens this profile for you."
  }
}
